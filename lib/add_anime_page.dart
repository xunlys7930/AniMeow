...
import 'ui/series_dialogs.dart'; // 新增
...
class _AddAnimePageState extends State<AddAnimePage> {
  ...
  /// 【新增】检查提醒设置完整性
  /// 返回 true 表示验证通过或用户决定不保存退出
  /// 返回 false 表示用户需要留在页面进行设置
  ...
  bool _hasUnsavedChanges() {
    ...
    Map<String, bool> diffs = {
      ...
      'rating': _rating != _initialSnapshot['rating'],
  ...
  void _saveAnime() async {
    ...
    try {
      ...
      // 【更新提醒】实际调度逻辑
      final finalId = animeData['id'];
      if (_isReminderEnabled && _reminderDay != null && _reminderTime != null) {
        await NotificationService().scheduleWeeklyNotification(
          id: finalId,
          title: '追番提醒',
          body: '您追的番剧《$title》今天更新啦，快去看看吧！',
          day: _reminderDay!,
          time: _reminderTime!,
        );
      } else {
      ...
      if (mounted) {
        ...
        setState(() => _isSaving = false); // 出错时解除锁
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) async {
        ...
        // 【最高优先级】如果是“发现”或“资源库搜索”进来的预览模式，且尚未入库，则不论是否有更改都直接退出
          ...
          title: Row(
            ...
            children: [
              Icon(
                ...
                size: 22,
                color: _subjectType == 'anime' ? Colors.blue : Colors.teal,
              ),
              const SizedBox(width: 10),
              Text(
                _isEditMode
                    ? (_subjectType == 'anime' ? '编辑番剧' : '编辑漫画')
                    : (_subjectType == 'anime' ? '添加番剧' : '添加漫画'),
              ),
            ],
          ),
          actions: [
            IconButton(
              onPressed: _isSaving ? null : _saveAnime, // 保存中禁用
              icon: _isSaving
                  ? const SizedBox(
                      width: 20,
                      height: 20,
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                // 类型选择
                AbsorbPointer(
                  absorbing: _isEditMode, // 编辑模式下只读
                  child: Opacity(
                    opacity: _isEditMode ? 0.8 : 1.0, // 编辑模式下稍微淡化
                ...
                const SizedBox(height: 16),
                _buildReminderCard(primaryColor),
                const SizedBox(height: 16),
                NeumorphicContainer(
                ...
                if (_seriesSiblings.isNotEmpty) ...[
                  const SizedBox(height: 16),
                  _buildSeriesNavigationSection(primaryColor),
                ],
                const SizedBox(height: 16),
                _buildDetailSection(primaryColor),
