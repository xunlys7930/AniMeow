...
class _AnimeDetailPageState extends State<AnimeDetailPage> {
  ...
  Future<void> _loadAll() async {
    ...
    if (anime['id'] is int) {
      tags = await db.getTagsByAnimeId(anime['id'] as int);
    }
  ...
  Future<void> _openEditor() async {
    ...
    final result = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => AddAnimePage(existingAnime: anime),
      ),
    );
    if (result == true) {
      _hasChanges = true;
                ...
                builder: (context, hidden, _) {
                  ...
                  return Scaffold(
                    ...
                    body: loading
                        ...
                        : _buildContent(layout, order, hidden, isMagazine),
                  );
                },
              );
            },
  ...
  Widget _buildContent(
    ...
    final visible =
        order.where((m) => !hidden.contains(m)).toList(growable: false);
    ...
    return SafeArea(
      bottom: false,
      child: _buildLayout(layout, props),
    );
  }
