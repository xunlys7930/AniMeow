...
import 'package:anime_tracker/theme_manager.dart';
import 'package:anime_tracker/settings_manager.dart';
import 'statistics_page.dart';
import 'fun_rating_page.dart';
...
import 'image_anime_search_page.dart';
import 'cloud_account_page.dart';
import 'package:anime_tracker/settings/settings_general_page.dart';
import 'package:anime_tracker/settings/settings_display_page.dart';
import 'package:anime_tracker/settings/settings_data_page.dart';
import 'package:anime_tracker/settings/settings_about_page.dart';
import 'package:anime_tracker/settings/testing_features_page.dart';
import 'package:anime_tracker/settings/reminder_management_page.dart';
import 'package:anime_tracker/ui/components/app_brand_icon.dart';
import 'package:anime_tracker/ui/design_tokens.dart';

/// 「我的」页面：4-Tab 重构后聚合统计、资料库、设置、主题、关于等次级入口。
///
/// 设计上分为 4 个分组：
/// 1. **数据中心**：统计 / 资料库 / 追番提醒
/// 2. **个性化**：主题 / 展示定制
...
class MyPageState extends State<MyPage> {
          ...
          SliverLayoutBuilder(
            builder: (context, constraints) {
              ...
              return SliverPadding(
                ...
                sliver: SliverToBoxAdapter(child: _buildHeader(colorScheme)),
              );
            },
          ),
          SliverLayoutBuilder(
                  ...
                  delegate: SliverChildListDelegate([
                    ...
                    _buildSettingsCard([
                      ...
                      _buildTile(
                        ...
                        title: '趣味评级',
                        subtitle: '制作专属番剧梯队榜并分享图片',
                        onTap: () => _push(const FunRatingPage()),
                      ),
                      _buildDivider(),
                      _buildTile(
                        ...
                        title: '角色管理',
                        subtitle: '角色资料、关联作品与关系星图',
                        onTap: () => _push(const CharacterManagementPage()),
                      ),
                      _buildDivider(),
                      _buildTile(
                        ...
                        title: '角色群组社区',
                        subtitle: '发现、预览并同步可复用的角色收藏',
                        onTap: () => _push(const CharacterCommunityPage()),
                      ),
                      _buildDivider(),
                      _buildTile(
                        ...
                        color: Colors.blue,
                        title: '账号与云同步',
                        subtitle: '可选云账号、云存储与恢复',
                        onTap: () => _push(const CloudAccountPage()),
                      ),
                      _buildDivider(),
                      _buildTile(
                        ...
                        color: Colors.green,
                        title: '以图搜番',
                        subtitle: '选择截图识别番剧并添加',
                        onTap: _openImageAnimeSearch,
                      ),
  ...
  Future<void> _openImageAnimeSearch() async {
    await Navigator.push<void>(
      context,
      MaterialPageRoute(
        builder: (_) => ImageAnimeSearchPage(
          useResultLabel: '添加此番剧',
          onSelection: (searchContext, selection) async {
            final added = await Navigator.of(searchContext)
                .pushReplacement<bool, void>(
                  MaterialPageRoute(
                    builder: (_) =>
                        AddAnimePage(initialImageSearchSelection: selection),
                  ),
                );
    ...
    showModalBottomSheet(
      ...
      builder: (sheetCtx) => Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('选择本命色', style: Theme.of(context).textTheme.headlineMedium),
            Text(
              '点击预设色或最后一格自定义 RGB',
              style: Theme.of(context).textTheme.bodySmall?.copyWith(
                color: Theme.of(context).colorScheme.onSurfaceVariant,
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: ColorPicker(
              ...
              pickerAreaBorderRadius: BorderRadius.circular(AppRadius.sm),
            ),
          ),
          actions: [
            TextButton(
    ...
    return InkWell(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: AppSpacing.sm),
          Text('自定义', style: Theme.of(context).textTheme.labelMedium),
        ],
      ),
    );
  }
